...
def import_data(filename):
    ...
    with open(filename, newline='') as csvfile:
        ...
        for row in csvreader:
            MAIN.append(row)
        for i in range(0,6):
            MAIN[i][2]= float(MAIN[i][2])
            MAIN[i][3]= float(MAIN[i][3])
    """
...
def fight(participant1, participant2, first2attack):
    ...
    rounds = 0
    attacker = first2attack
    while(HP1>=0 and HP2>=0):
        if (attacker == 1):
            HP2 = HP2 - ATK1 * attack_multiplier(Type1,Type2)
            attacker = 2
        else:
            HP1 = HP1 - ATK2 * attack_multiplier(Type2,Type1)
            attacker = 1
        rounds = rounds +1
    if (HP1 > 0):
        return [1,rounds]
